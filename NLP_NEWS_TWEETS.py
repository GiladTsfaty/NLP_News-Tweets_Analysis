...
cnn_tweets = pd.read_csv('tweets_cnn.csv', low_memory=False)
bbc_tweets = pd.read_csv('tweets_bbc.csv', low_memory=False)
...
#####  WORD2VEC  #####
def preprocess_text4(tokens):
    return [token.lower() for token in tokens if token.lower() not in stopwords3 and token.isalpha() and token != 'http']
...
# similar_words_2 = w2v_model.wv.most_similar('trump')

# print("Most similar words to 'say':", similar_words_2)
